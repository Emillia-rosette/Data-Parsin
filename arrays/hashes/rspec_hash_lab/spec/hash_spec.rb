...
describe 'Create tests for a hash' do
  ...
  it 'each fruit should have 4 keys' do
    # expect(small_fruit_nutrition_details.keys.length).to be eq 4
        small_fruit_nutrition_details.each do |k, v|
        expect(v.keys.length).to eq 4
      end
      

    # expect(small_fruit_nutrition_details[:avocado].keys).to be eq 4
    # expect(small_fruit_nutrition_details[:kiwifruit].keys).to be eq 4
    # expect(small_fruit_nutrition_details[:plums].keys).to be eq 4
  end
  ...
  it 'all calories should be between 40 to 150' do
    ...
    # expect(small_fruit_nutrition_details.each).to be > 40
    # expect(small_fruit_nutrition_details.each).to be < 150
